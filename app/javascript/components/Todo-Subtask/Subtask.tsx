...
const Subtask = ({
  ...
  useEffect(() => {
    if (loaded) {
      ...
      axios
        .patch(url, { done: subtaskBool })
        .catch((resp) => console.log(resp));
    }
  }, [subtaskBool]);

  const handleChangeSubtask = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSubtasktxt(e.target.value);
  };

  useEffect(() => {
    if (loaded) {
      const url = `/api/v1/subtasks/${id}`;
      axios.patch(url, { text: subtasktxt }).catch((resp) => console.log(resp));
    }
  }, [debouncedSubtasktxt]);
  ...
  return (
    <Wrapper
      style={{
        backgroundColor: subtaskBool ? "rgb(188, 255, 182)" : "#edf5ff",
      }}
    >
      <Checkbox
        style={{
