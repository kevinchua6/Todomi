...
const CardTags = ({ tags, screenWidth, handleDelete }: CardTagsI) => {
  ...
  const getNumOfTagsToDisplay = (
    tags: Tag[],
    maxWidth: number,
    lenBtwnChips: number
  ) => {
    let numOfTagsToDisplay = 0,
      currentLen = tags[0].attributes.name.length + lenBtwnChips;
  ...
  const numOfTagsToDisplay: number = getNumOfTagsToDisplay(
    tagsDisplayed,
    columnWidth - 70,
    14
  );
      ...
      {tagsHidden.length > 0 && (
        <Fragment>
          <Tooltip title="Click to view more tags">
            <Chip size="small" label="..." onClick={handleClick} />
          </Tooltip>
          <Popover
