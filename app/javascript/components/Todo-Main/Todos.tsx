...
const Todos = ({
  ...
  const [inputTodo, setInputTodo] = useState<InputTodo>({
    title: "",
    date: null,
  });
  ...
  useEffect(() => {
    axios
      ...
      .then((resp) => {
        ...
        const todosArr: Todos[] = rawData.data.filter(
          (todo: Todos) => todo.attributes.user_id === userId
        );
        setTodos(todosArr);
      })
  ...
  const handleCompleteTodo = (todo_id: string) => {
    axios
      .patch(`/api/v1/todos/${todo_id}`, { done: true })
      .then((resp) => {
        setTodos(
          todos.map((todo) =>
            todo.id === todo_id
  ...
  const handleRestoreTodo = (todo_id: string) => {
    axios
      .patch(`/api/v1/todos/${todo_id}`, { done: false })
      .then((resp) => {
        setTodos(
          todos.map((todo) =>
            todo.id === todo_id
    ...
    const deleteTask = async () => {
      ...
      setTags(
        rawData.included.filter(
          (tag: Tag) =>
            tag.type === "tag" && todoIdArr.includes(tag.attributes.todo_id)
        )
      );
    };
    ...
    .map((todo, index) => {
      ...
      return (
        <div
          key={todo_id}
          style={{
            backgroundColor: isDue ? "#ff8c8c" : "#91c5ff",
            borderRadius: 5,
          }}
  ...
  const inputTodoHandleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    <Fragment>
      ...
      <Home>
        ...
        {currentTab === "Homepage" && noHomepageTodos === 0 ? (
          ...
          <Notifications>
            {" "}
            You have no Completed Tasks! Go accomplish something!{" "}
          </Notifications>
        ) : loaded ? (
