...
const Todo = ({
  ...
  const conventionalDueDate = dueDate
    ? convertToConventionalDate(dueDate)
    : null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const newSubtasks = rawData.included.filter(
  ...
  useEffect(() => {
    ...
    setRenderSubtasks(
      [...undoneSubtasks, ...doneSubtasks].map((subtask, index) =>
        index < maxNoSubtask ? (
          <TodoSubtask
            key={subtask.id}
            id={subtask.id}
  ...
  const renderDueDate =
    dueDate && isDue ? (
      <DueDateWrapper>Due on {conventionalDueDate}!!!</DueDateWrapper>
    ) : dueDate ? (
      <DueDateWrapper>Due on {conventionalDueDate}</DueDateWrapper>
    ) : (
      <EmptyDueDateWrapper />
